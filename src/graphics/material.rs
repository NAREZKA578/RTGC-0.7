...
impl Material {
    ...
    pub fn load(
        ...
        let material_path = format!("{}.material.toml", path);
        
        // Для альфы создаём дефолтный материал
        let mut material = Self::new(path);
        material.loaded_quality = quality;
        
        // Попытаться загрузить описание материала из TOML
        if Path::new(&material_path).exists() {
            // В полной версии здесь был бы парсинг TOML
            // Для альфы используем заглушку
        }
        
        // Загрузить базовую текстуру albedo (всегда)
        let albedo_path = format!("{}_albedo.dds", path);
        if Path::new(&albedo_path).exists() {
            material.layers.albedo = Some(loader.load_texture(&albedo_path)?);
        }
        
        // Загрузить normal map если поддерживается
        if quality.supports_normal() {
            let normal_path = format!("{}_normal.dds", path);
            if Path::new(&normal_path).exists() {
                material.layers.normal = Some(loader.load_texture(&normal_path)?);
            }
        }
        
        // Загрузить ORM texture если поддерживается
        if quality.supports_orm() {
            let orm_path = format!("{}_orm.dds", path);
            if Path::new(&orm_path).exists() {
                material.layers.orm = Some(loader.load_texture(&orm_path)?);
            }
        }
        
        // Загрузить emissive если поддерживается
        if quality.supports_emissive() {
            let emissive_path = format!("{}_emissive.dds", path);
            if Path::new(&emissive_path).exists() {
                material.layers.emissive = Some(loader.load_texture(&emissive_path)?);
            }
        }
        
        // Загрузить detail normal только для Ultra
        if quality.supports_detail() {
            let detail_path = format!("{}_detail_normal.dds", path);
            if Path::new(&detail_path).exists() {
                material.layers.detail = Some(loader.load_texture(&detail_path)?);
            }
        }
    ...
    pub fn upgrade(&mut self, new_quality: TextureQuality, loader: &mut AssetLoader) -> Result<(), MaterialLoadError> {
        if new_quality <= self.loaded_quality {
            return Ok(()); // Уже достаточно хорошего качества
        }
        
        // Загрузить недостающие текстуры
        if new_quality.supports_normal() && !self.loaded_quality.supports_normal() {
            // Загрузить normal map
            // (путь нужно хранить в материале - для альфы заглушка)
        }
        
        if new_quality.supports_orm() && !self.loaded_quality.supports_orm() {
            // Загрузить ORM
        }
        
        if new_quality.supports_emissive() && !self.loaded_quality.supports_emissive() {
            // Загрузить emissive
        }
        
        if new_quality.supports_detail() && !self.loaded_quality.supports_detail() {
            // Загрузить detail normal
        }
