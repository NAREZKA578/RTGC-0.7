...
impl RendererRhi {
    ...
    fn create_bitmap_font(device: &Arc<dyn IDevice>) -> Result<(ResourceHandle, HashMap<char, [f32; 4]>), Box<dyn std::error::Error>> {
        ...
        let texture = device.create_texture(&desc, Some(&pixels))?;
    ...
    fn render_game(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Render HUD
        if let Some(ref hud_data) = self.hud_data {
            self.render_hud(hud_data)?;
        }
    ...
    fn render_hud(&mut self, hud_data: &crate::ui::hud::VehicleHudData) -> Result<(), Box<dyn std::error::Error>> {
