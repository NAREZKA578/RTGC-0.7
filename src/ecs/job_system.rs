...
impl JobSystem {
    ...
    /// Параллельное выполнение итерации
        ...
        F: Fn(I::Item) + Send + Sync + 'static,
    {
        ...
        for item in items {
            ...
            let job = ParallelJob::new(move || f(item));
            let id = self.add_job(job, JobPriority::Normal);
            job_ids.push(id);
        }
        
        // Ожидание завершения всех задач
        let _ = self.wait_for_jobs(&job_ids, Some(std::time::Duration::from_secs(60)));
    }
}
