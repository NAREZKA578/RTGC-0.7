...
impl SystemScheduler {
    ...
    pub fn update_all(&mut self, world: &mut World, delta_time: Duration) {
    ...
    pub fn post_update_all(&mut self, world: &mut World) {
        for (i, system) in self.systems.iter_mut().enumerate() {
            if self.enabled[i] {
...
impl System for TestSystem {
    ...
    fn update(&mut self, _world: &mut World, _delta_time: Duration) {
        self.update_count += 1;
    }
}
