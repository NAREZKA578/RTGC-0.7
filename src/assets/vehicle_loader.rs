...
use crate::physics::{PhysicsWorld, RigidBody, Vehicle, VehicleConfig, WheelState};
...
impl VehicleLoader {
    ...
    pub fn to_vehicle_config(def: &VehicleDefinition) -> VehicleConfig {
        VehicleConfig {
            ...
            engine_power: def.engine.max_power_kw * 1000.0, // kW -> W
            max_rpm: def.engine.max_rpm,
            idle_rpm: def.engine.idle_rpm,
            gear_ratios: def.transmission.gear_ratios.clone(),
            reverse_ratio: def.transmission.reverse_ratio,
            final_drive: def.transmission.final_drive,
            wheel_count: def.wheels.len() as u8,
            steer_angle: def.wheels.iter()
