...
impl SpringConstraint {
    ...
    pub fn current_length(&self, world: &PhysicsWorld) -> f32 {
        if let (Some(body_a), Some(body_b)) = (
            world.rigid_bodies.get(self.body_a),
            world.rigid_bodies.get(self.body_b),
        ) {
...
impl RaycastSuspension {
    ...
    pub fn get_attachment_point_world(&self, world: &PhysicsWorld) -> Option<Vector3<f32>> {
        world.rigid_bodies.get(self.chassis_body).map(|body| {
...
impl PhysicsWorld {
    ...
    pub fn solve_spring_constraints(&mut self, dt: f32) {
        ...
        for constraint in &self.spring_constraints {
            if let (Some(body_a), Some(body_b)) = (
                self.rigid_bodies.get(constraint.body_a),
                self.rigid_bodies.get(constraint.body_b),
            ) {
        ...
        for (body_idx, force) in constraints_to_apply {
            if let Some(body) = self.rigid_bodies.get_mut(body_idx) {
                if !body.is_static {
...
pub use crate::physics::physics_module::PhysicsWorld;
