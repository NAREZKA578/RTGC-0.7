...
impl RigidBody {
    ...
    pub fn update(&mut self, dt: f32) {
        if !self.is_static {
            ...
            // Integrate torques
            let angular_acceleration = self.inverse_inertia_tensor * self.torques;
...
impl PhysicsWorld {
    ...
    pub fn add_body(&mut self, body: RigidBody) -> usize {
        let index = self.rigid_bodies.len();
        self.rigid_bodies.push(body);
        index
    }

    pub fn remove_body(&mut self, index: usize) -> Option<RigidBody> {
        if index < self.rigid_bodies.len() {
            Some(self.rigid_bodies.remove(index))
        } else {
            None
        }
    }
    ...
    fn broadphase_collision_detection(&mut self) {
        ...
        // Simple all-vs-all check with AABB culling
