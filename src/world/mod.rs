...
impl OpenWorld {
    ...
    pub fn new(seed: u64) -> Self {
        ...
        Self {
            ...
            load_radius: streaming_config.load_radius,
            unload_radius: streaming_config.unload_radius,
        }
    }
    ...
    fn process_loaded_chunks(&mut self) {
        while let Some(chunk) = self.streamer.poll_loaded_chunk() {
            let chunk_id = chunk.id;
            debug!("Chunk {:?} finished loading", chunk_id);
            
            // Insert into loaded chunks
            let arc_chunk = Arc::new(chunk);
            self.loaded_chunks.insert(chunk_id, arc_chunk.clone());
            
            // Add to spatial index
            self.spatial_index.insert_chunk(arc_chunk);
        }
    }
