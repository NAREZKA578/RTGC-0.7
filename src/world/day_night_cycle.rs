...
impl DayNightCycle {
    ...
    pub fn get_celestial_view_matrix(&self, body: CelestialBody, distance: f32) -> Matrix4<f32> {
        let position = match body {
            CelestialBody::Sun => self.sun.position.normalize() * distance,
            CelestialBody::Moon => self.moon.position.normalize() * distance,
        };
        
        Matrix4::look_at_rh(&position, &Vector3::new(0.0, 0.0, 0.0), &Vector3::new(0.0, 1.0, 0.0))
    }
}
